...
async function dashboard() {
  ...
  async function render() {
    ...
    const latest = overview.latestPrescription || rxData.items[0];
    const next = overview.nextMedication || medData.items[0];
    ...
    qs('content').innerHTML = `
      <div class="grid-3">
        ...
        <article class="card">
          ...
          <h3>Notifications</h3>
          <p class="muted">${overview.unread} unread</p>
          <p><a href="${hrefTo('patient/notifications.html')}">View all</a></p>
        </article>
      </div>
      <div class="list card-stack">
        ${overview.appointments.map((row) => `<div class="item"><div><strong>${h(row.doctor_name)}</strong><div class="tiny">${formatWhen(row.appointment_start)} · ${h(row.type)}</div></div>${badge(row.status)}</div>`).join('') || '<div class="empty">No appointments to show.</div>'}
      </div>`;
  ...
  qs('content').innerHTML = `
    <form id="profileForm" class="card">
      <div class="form-row">
        ...
        <div class="field"><label>Email</label><input value="${session.user.email}" disabled></div>
      </div>
      <div class="form-row">
        <div class="field"><label>Date of birth</label><input name="date_of_birth" type="date" value="${p.date_of_birth || ''}"></div>
        <div class="field"><label>Phone</label><input name="phone" value="${p.phone || ''}"></div>
      </div>
      <div class="field"><label>Address</label><input name="address" value="${p.address || ''}"></div>
      <div class="form-row">
        <div class="field"><label>Emergency contact</label><input name="emergency_contact_name" value="${p.emergency_contact_name || ''}"></div>
        <div class="field"><label>Emergency phone</label><input name="emergency_contact_phone" value="${p.emergency_contact_phone || ''}"></div>
      </div>
